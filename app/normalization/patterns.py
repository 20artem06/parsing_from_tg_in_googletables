...
STORAGE_RE = re.compile(r"\b(\d{2,4})\s*(tb|gb|гб|тб)\b", re.IGNORECASE)
...
IPHONE_RE = re.compile(
    r"\biphone\s*(\d{2})(?:\s*(pro max|pro|plus|max|mini|e))?\b",
    re.IGNORECASE,
)
IPAD_RE = re.compile(
...
MACBOOK_RE = re.compile(
    r"\bmacbook(?:\s+(air|pro))?(?:\s+(\d{1,2}(?:\.\d)?))?\b",
    re.IGNORECASE,
)
IMAC_RE = re.compile(r"\bimac(?:\s+(\d{2}))?\b", re.IGNORECASE)
WATCH_RE = re.compile(
    r"\b(?:apple\s+watch|watch)\s*(series\s*\d+|ultra\s*\d*|se\s*\d*)?\b",
    re.IGNORECASE,
)
AIRPODS_RE = re.compile(r"\bairpods(?:\s+(pro|max))?(?:\s*(\d))?\b", re.IGNORECASE)
